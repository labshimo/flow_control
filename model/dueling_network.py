...
class DuelingNetwork(tf.keras.Model):
    def __init__(self, config):
        ...
        self.use_image_layer = True if len(config.input_shape)>3 else False
        self.action_space    = config.action_space
        if self.use_image_layer:
            self.c1   = kl.Conv2D(32, 8, strides=4, activation="relu",
                               kernel_initializer="he_normal")
            self.c2   = kl.Conv2D(64, 4, strides=2, activation="relu",
                               kernel_initializer="he_normal")
            self.c3   = kl.Conv2D(64, 3, strides=1, activation="relu",
                               kernel_initializer="he_normal")
            self.fltn = kl.Flatten()
        else:
            self.dense    = kl.Dense(32, activation="relu", name="dense0", kernel_initializer="he_normal")
...
if __name__ == "__main__":
    ...
    actions       = [0, 1, 1]
    target_values = [1, 1, 1]
    qnet          = DuelingNetwork((None,4),action_space=2)
    ...
    qnet = DuelingNetwork((None, 84, 84, 4), action_space=4)
    #print(qnet.summary())
